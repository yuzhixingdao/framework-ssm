package com.aac.framework.common.util.bean;
...
public class BeanUtil {
	...
	public static <OBJECT, TOOBJECT> List<TOOBJECT> object2object(List<OBJECT> objectList,
			Class<TOOBJECT> toobjectClass) {
		if (objectList != null) {
	 ...
	 * @param listArray
	 * @param pojoClass
	 * @param setNameAndClass
	 *            odd setName, even class
	 * @return
	 */
				...
				if (leng > 0) {
					...
					try {
						...
						for (int j = 1; j < leng; j++) {
							Method method = pojo.getClass().getMethod(setNameAndClass[(j - 1) * 2].toString(),
									(Class) setNameAndClass[(j - 1) * 2 + 1]);
		...
		for (PropertyDescriptor targetPd : targetPds) {
			...
			if (writeMethod != null && (ignoreList == null || !ignoreList.contains(targetPd.getName()))) {
				...
				if (sourcePd != null) {
					...
					if (readMethod != null && readType.getName().equals(writeType.getName())) {
						try {
